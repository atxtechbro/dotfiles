#!/usr/bin/env python3
"""
Update README.md with an auto-generated memory section based on recent git activity.
This helps maintain context across Claude CLI sessions (anti-alzheimers, no 50 first dates).
"""
...
def generate_memory_section(days=7):
    ...
    # Most active areas
    if files[:10]:  # Top 10 files
        section.append(f"\n### Most Active Files")
        for count, filepath in files[:10]:
    ...
    # Recent feature work
    section.append(f"\n### Recent Feature Work")
    feature_commits = [c for c in commits if c['message'].startswith(('feat', 'feature'))][:5]
    if feature_commits:
    ...
    # Current areas of focus (based on file patterns)
    section.append(f"\n### Current Focus Areas")
...
def update_readme(memory_section):
    ...
    if re.search(memory_pattern, content, re.DOTALL):
        ...
        # Insert after the philosophy section, before modular shell config
...
def main():
    ...
    print(f"Analyzing git activity from the last {days} days...")
    memory_section = generate_memory_section(days)
    
    if not memory_section:
        print("No recent git activity found.")
        sys.exit(0)
    
    print("Generated memory section:")
